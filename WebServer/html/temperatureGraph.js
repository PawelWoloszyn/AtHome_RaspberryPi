  ...
  $(document).ready(function(){
      $.ajax({
          ...
          success: function(response){
            dataArray = parseDBdata(response);
...
function parseDBdata(response){
    var array = [];
    for (var i = response.length-1; i >= 0 ; i--){
      var dataItem = [];
      dataItem[0] = new Date(response[i]["evtDate"]);
      dataItem[1] = parseFloat(response[i]["outdoorTmp"]);
      dataItem[2] = parseFloat(response[i]["ovenTmp"]);
      if(!isNaN(dataItem[1]))
        array.push(dataItem);
    }
    return array;
...
function drawChart() {
    ...
    data.addColumn('number', 'Temperatura na dworze');
    data.addColumn('number', 'Temperatura pieca');
	...
	series: {
        ...
        vAxes: {
           0: { title: "Temperatura na dworze"},
	   1: { title: "Temperatura pieca"}
        }
    };
...
function displayTemperatures() {
  var tempDiv = document.getElementById("currentTemperatures");
  if(dataArray.length > 0){
    var lastElement = dataArray.length - 1;
    tempDiv.textContent ="dwor: " + (dataArray[lastElement][1]).toString() + " piec: " + (dataArray[lastElement][2]).toString();
  }else{
    tempDiv.textContent ="dwor: ?" + " piec: ?";
  }
}
...
document.getElementById("refresh").onclick = function() {
    viewportEndTime = new Date(); // the end time of the viewport is the current time
    loadDBdata(); // download the latest data and re-draw the chart
}
